...
-(id)initWithGroupId:(NSString*)groupId index:(NSUInteger)index size:(CGSize)frame{
    //调用初始化函数
    self = [self init];
    if (self) {
...
//初始化按钮
-(void)defaultInit : (CGSize)frame{
    //设置控件自己的Frame
    self.frame = CGRectMake(0, (44- frame.height)/2, frame.width, frame.height);
    
    // 客户定制按钮
    _button = [UIButton buttonWithType:UIButtonTypeCustom];
    _button.frame = CGRectMake(0, 3,frame.width,frame.height);
    //一个布尔值,决定了图像的变化当按钮高亮显示。
    _button.adjustsImageWhenHighlighted = NO;
    
    //正常状态下的按钮背景图
    [_button setBackgroundImage:[UIImage imageNamed:@"Home_Access-nochoose"]forState:UIControlStateNormal];
    [_button setBackgroundImage:[UIImage imageNamed:@"Home_Access-Sexselected"]forState:UIControlStateSelected];
...
-(void)handleTitle:(NSString *)string{
    _button.titleLabel.font = [UIFont systemFontOfSize:12];
    [_button setTitleColor:fRgbColor(128, 128, 128) forState:UIControlStateNormal];
    [_button setTitleColor:fRgbColor(166, 189, 65) forState:UIControlStateSelected];
