...
static const CGFloat kWidthOfButtonMargin =00.0f;// 32.0f;
...
{
    //创建顶部可滑动的tab
    _topScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.frame.size.height)];
    kHeightOfTopScrollView=self.frame.size.height;
...
//    均等分宽度，
    if (_viewArray.count * kWidthOfBtnName<kNMDeviceWidth) {
        if (!_viewArray.count) {
            kWidthOfBtnName=kNMDeviceWidth;
        }else{
            kWidthOfBtnName=kNMDeviceWidth/_viewArray.count;
        }
...
//        CGFloat systemFont = self.titleFont ==0? kFontSizeOfTabButton:self.titleFont;
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        //        NSDictionary *attribute = @{NSFontAttributeName: [UIFont systemFontOfSize:kFontSizeOfTabButton]};
        //        CGSize textSize = [title boundingRectWithSize:CGSizeMake(_topScrollView.bounds.size.width, kHeightOfTopScrollView) options: NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
        CGSize textSize=CGSizeMake(kWidthOfBtnName, 40);
        ...
        //设置按钮尺寸
        float btnX=(float)xOffset;
        if (!btnX) {
            btnX=0;
        }
        float btnW=(float)textSize.width;
        if (!btnW) {
            btnW=kNMDeviceWidth/3;
        }
        [button setFrame:CGRectMake(btnX,0,btnW,kHeightOfTopScrollView)];
        //计算下一个tab的x偏移量
        xOffset += textSize.width + kWidthOfButtonMargin;
        
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(0, kHeightOfTopScrollView-_shadowImage.size.height, textSize.width, 10);
            button.selected = YES;
        }
        [button setTitle:title forState:UIControlStateNormal];
        [button setTitle:title forState:UIControlStateSelected];
        button.titleLabel.lineBreakMode =  NSLineBreakByTruncatingTail;//让多余的文字省略号在中间
...
//        }
//        rightLine.frame=CGRectMake(xOffset-0.25, 0, 0.5, kHeightOfTopScrollView);
//        [_topScrollView addSubview:rightLine];
//        [rightLine mas_makeConstraints:^(MASConstraintMaker *make) {
//            make.width.mas_equalTo(@0.5);
//            make.height.mas_equalTo();
//            make.left.equalTo(_topScrollView.mas_left).with.offset(i*textSize.width);
...
//                                 lineBreakMode:NSLineBreakByTruncatingTail];
//         //累计每个tab文字的长度
//         allTitlesWidth += kWidthOfButtonMargin+textSize.width;
//     }
//    CGFloat  firstButtonMargin=(kGTSDeviceWidth-allTitlesWidth)/2;
//    CGFloat topScrollViewContentWidth = firstButtonMargin-(kWidthOfButtonMargin/2);
//    
//    for (int i = 0; i < [_viewArray count]; i++) {
//       // UIViewController *vc = _viewArray[i];
//        NSString * title=self.titleArray[i];
...
//        
//        //设置按钮尺寸
//        [button setFrame:CGRectMake(topScrollViewContentWidth,0,
...
//        [button setTag:i+100];
//        if (i == 0) {
//            _shadowImageView.frame = CGRectMake(topScrollViewContentWidth, kHeightOfTopScrollView-_shadowImage.size.height, textSize.width, _shadowImage.size.height);
//            button.selected = YES;
//        }
//        [button setTitle:title forState:UIControlStateNormal];
//        button.titleLabel.font = [UIFont systemFontOfSize:kFontSizeOfTabButton];
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x, kHeightOfTopScrollView-_shadowImage.size.height, sender.frame.size.width, _shadowImage.size.height)];
